...
const ScanLogs = () => {
  ...
  const tokenn = params.soz; 
  const id = tokenn ? atob(tokenn) : null;


  const fetchLogs = async () => {
    try {
      const token = await getItem("token");
      const response = await fetch(`${BASE_URL}/charm/singleScanLog/${id}`, {
  ...
  const deleteSelectedLogs = async () => {
    ...
    triggerLocalNotification(
      ...
      `Are you sure you want to delete ${selectedLogs.size} scan log${selectedLogs.size > 1 ? 's' : ''}?`,
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              await Promise.all(
                selectedIds.map(id => 
                  fetch(`${BASE_URL}/charm/deleteScanLog/${id}`, {
                    method: "DELETE",
                    headers: { Authorization: token },
                  })
                )
              ...
              triggerLocalNotification("Deleted", `${selectedIds.length} scan logs deleted`);
              exitSelectionMode();
            } catch (error) {
              console.log("Error deleting selected logs:", error);
              triggerLocalNotification("Error", "Failed to delete scan logs");
            }
          }
        },
      ]
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              const response = await fetch(`${BASE_URL}/charm/deleteAllScanLogs`, {
                method: "DELETE",
                headers: { Authorization: token },
              });
              ...
              if (response.ok) {
                setLogs([]);
                triggerLocalNotification("Deleted", "All scan logs deleted");
                exitSelectionMode();
              } else {
                // Fallback: delete individually
                const deletePromises = logs.map(log => 
                  fetch(`${BASE_URL}/charm/deleteScanLog/${log.id}`, {
                    method: "DELETE",
                    headers: { Authorization: token },
                  })
                );
              ...
              console.log("Error deleting all logs:", error);
              triggerLocalNotification("Error", "Failed to delete all scan logs");
            }
          }
        },
      ]
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.footerText}>Refresh the page</Text>
    </View>
  );
        ...
        >
          ...
          {selectionMode && (
            <TouchableOpacity
              style={[
                styles.checkbox,
                selectedLogs.has(log.id) && styles.checkboxSelected
              ]}
          ...
          <View style={styles.headerLeft}>
            ...
            <View>
              <Text style={styles.headerTitle}>
                {selectionMode ? `${selectedLogs.size} selected` : "Scan Analytics"}
              </Text>
              {!selectionMode && (
                <Text style={styles.headerSubtitle}>
                  {logs.length} total scan{logs.length !== 1 ? 's' : ''}
                </Text>
              )}
            </View>
          </View>
            ...
            {selectionMode ? (
              <>
                <TouchableOpacity 
                  ...
                  onPress={deleteSelectedLogs}
                  disabled={selectedLogs.size === 0}
                >
                  <Icon name="trash" size={22} color={selectedLogs.size === 0 ? "#9CA3AF" : "#EF4444"} />
                </TouchableOpacity>
              </>
            ) : (
              <>
                {logs.length > 0 && (
                  <>
                    ...
                    >
                      <Icon name="check-square" size={22} color="#4F46E5" />
                    </TouchableOpacity>
                    <TouchableOpacity 
                      style={styles.headerActionButton}
        ...
        {selectionMode && (
          <View style={styles.selectionActions}>
            <TouchableOpacity 
              ...
              onPress={selectAll}
            >
              <Icon name="check-square" size={18} color="#4F46E5" />
            ...
            <TouchableOpacity 
              ...
              disabled={selectedLogs.size === 0}
            >
              <Icon name="trash" size={18} color={selectedLogs.size === 0 ? "#9CA3AF" : "#EF4444"} />
              <Text style={[
                styles.selectionActionText,
                selectedLogs.size === 0 && styles.selectionActionTextDisabled
              ]}>
...
const styles = StyleSheet.create({
  ...
  // Filter Tabs - Ultra Compact Modern
  filterScrollView: {
    marginBottom: 20,
    height: 36,
  },
